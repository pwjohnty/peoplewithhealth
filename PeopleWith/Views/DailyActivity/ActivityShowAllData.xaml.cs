...
public partial class ActivityShowAllData : ContentPage
{
    ...
    public userdailyactivity ActivityPassed = new userdailyactivity(); 
    ...
    public ActivityShowAllData(ObservableCollection<userdailyactivity> AllActivity, userdailyactivity PassedActivity)
    {
        InitializeComponent();

        AllUserActivities = AllActivity;
        ActivityPassed = PassedActivity;
        Titlelbl.Text = ActivityPassed.activitytitle;
    ...
    private void FilterItems()
    {
        try
        {
            var Filter = AllUserActivities.Where(x => x.activitytitle == ActivityPassed.activitytitle).ToList();

            foreach (var item in Filter)
            {
                if (String.IsNullOrEmpty(item.ActivityFeedbackList.Completed))
                {
                    item.ActivityFeedbackList.Completed = "Not Recorded";
                }
                else
                {
                    if(item.ActivityFeedbackList.Completed == "Not Completed")
                    {
                        item.activityplanner = false; 
                    }
                }
                if (String.IsNullOrEmpty(item.ActivityFeedbackList.Mood))
                {
                    item.ActivityFeedbackList.Mood = "None";
                }
                if (String.IsNullOrEmpty(item.ActivityFeedbackList.Outcome))
                {
                    item.ActivityFeedbackList.Outcome = "None";
                }
            ...
            AllDataLV.ItemsSource = Filter.OrderByDescending(m => DateTime.Parse(m.startdate));

            if(Filter.Count > 0)
            {
                ListviewStack.IsVisible = true; 
                FilterStackEmpty.IsVisible = false;
            }
            else
            {
                ListviewStack.IsVisible = true; 
                FilterStackEmpty.IsVisible = true; 
            }
        }
        catch (Exception Ex)
        {

...
{
    public class MedSuppFeedbackCoventer : JsonConverter
    {
        public ObservableCollection<MedSuppFeedback> FeedbackSymptoms = new ObservableCollection<MedSuppFeedback>();
        public string ReturnString;
        public override bool CanConvert(Type objectType)
        {
            return objectType == typeof(ObservableCollection<MedSuppFeedback>);
        }
            ...
            try
            {
                ...
                if (token.Type == JTokenType.String)
                {
                    ...
                    if (value.Contains("id"))
                    {
                        ...
                        var AddFeedback = new MedSuppFeedback();
                        AddFeedback.id = timesubstring[1];
                        AddFeedback.Recorded = SymFeedback[1];
                        AddFeedback.datetime = Intensity[1];
                    ...
                    {
                        return new ObservableCollection<MedSuppFeedback>();
                    }
                }
